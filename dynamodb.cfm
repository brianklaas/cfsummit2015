<cfset dynamoResult = "" />
<cfset tableNameInDynamoDB = "YOUR DYNAMO TABLE NAME GOES HERE" />

<cfif structKeyExists(URL, "goDynamo")>
	<cfscript>
	dynamoDB = CreateObject('component', 'cfsummit2015.model.awsServiceFactory').init().createServiceObject('dynamoDB');
	...
	switch(lcase(trim(URL.goDynamo))){
		...
		case 'putItem':
			...
			itemMaker = CreateObject('component','cfsummit2015.model.dynamoItemMaker').init();
